...
const catalogMenu = `BookCatalog's menu:
	1 -- view books
	2 -- next page
...
func (r *Requester) ProcessBookCatalogActions() error {
	...
	r.cache.Set(bookParamsKey, dto.BookParamsDTO{Limit: impl.PageLimit, Offset: 0})
...
func (r *Requester) viewFirstPage() error {
	...
	bookParams.Limit = impl.PageLimit
	bookParams.Offset = 0
	request := HTTPRequest{
		...
		QueryParams: map[string]string{
			...
			"page_number":     fmt.Sprintf("%d", bookParams.Offset/int(bookParams.Limit)+1),
		},
	...
	r.cache.Set(
		bookParamsKey,
		dto.BookParamsDTO{
			Limit:  impl.PageLimit,
			Offset: bookParams.Offset + impl.PageLimit,
		},
	)
...
func (r *Requester) getAvgRatingForBook(bookID uuid.UUID) (float32, error) {
	request := HTTPRequest{
		Method: http.MethodGet,
		URL:    r.baseURL + fmt.Sprintf("/books/%s/ratings/avg", bookID.String()),
		Headers: map[string]string{
			"Content-Type": "application/json",
		},
		QueryParams: map[string]string{
...
func (r *Requester) viewBookRatings() error {
	...
	request := HTTPRequest{
		Method: http.MethodGet,
		URL:    r.baseURL + fmt.Sprintf("/books/%s/ratings", bookID.String()),
		Headers: map[string]string{
			"Content-Type": "application/json",
		},
		QueryParams: map[string]string{
...
func printBooks(books []*jsonmodels.BookModel, offset int) {
	t := table.NewWriter()
	t.SetTitle(fmt.Sprintf("Страница книг №%d", offset/impl.PageLimit+1))
	t.SetStyle(table.StyleBold)
	t.Style().Format.Header = text.FormatTitle
