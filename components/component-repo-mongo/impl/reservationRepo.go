...
func (rr *ReservationRepo) GetExpiredByReaderID(ctx context.Context, readerID uuid.UUID) ([]*models.ReservationModel, error) {
	...
	filter := bson.M{
		"$or": []bson.M{
			{"return_date": bson.M{"$lte": time.Now().Format("2006-01-02")}},
			{"state": "Expired"},
		},
		"reader_id": readerID,
	}
...
func (rr *ReservationRepo) GetByReaderID(ctx context.Context, readerID uuid.UUID, limit, offset int) ([]*models.ReservationModel, error) {
	rr.logger.Infof("find reservation with readerID: %s", readerID)
	...
	cursor, err := rr.db.Find(ctx, filter, findOptions)
	if err != nil {
		rr.logger.Errorf("error find expired reservations: %v", err)
		return nil, err
	}
	...
	if len(coreReservations) == 0 {
		rr.logger.Warnf("reservations with this readerID not found: %s", readerID)
		return nil, errs.ErrReservationDoesNotExists
	}

	rr.logger.Infof("found reservation with readerID: %s", readerID)
