...
func (h *Handler) getPageBooks(c *gin.Context) {
	...
	if h.isNoEmptyField(c.Query("page_number")) {
		...
		params.Limit = impl.BooksPageLimit
		params.Offset = int((pageNumber - 1) * impl.BooksPageLimit)
	}
...
func (h *Handler) getBookByID(c *gin.Context) {
	...
	bookOutputDTO, err := h.convertToBookOutputDTO(book)
	...
	c.JSON(http.StatusOK, bookOutputDTO)
}
...
// @Produce  json
// @Param id path string true "Идентификатор книги"
// @Param page_number query string true "Номер страницы"
...
func (h *Handler) getRatingsByBookID(c *gin.Context) {
	...
	if h.isNoEmptyField(c.Query("page_number")) {
		...
		limit = impl.ReviewsPageLimit
		offset = int((pageNumber - 1) * impl.ReviewsPageLimit)
	}

	ratings, err := h.ratingService.GetByBookID(c.Request.Context(), bookID, limit, offset)
	...
	fmt.Println(ratingOutputDTOs)
	fmt.Println(len(ratingOutputDTOs))
...
func (h *Handler) convertToBookOutputDTO(book *models.BookModel) (*dto.BookOutputDTO, error) {
	avgRating, err := h.ratingService.GetAvgRatingByBookID(context.Background(), book.ID)
	if err != nil && !errors.Is(err, errs.ErrRatingDoesNotExists) {
