...
func (h *Handler) getReservationsByReaderID(c *gin.Context) {
	fmt.Println("call getPageReservations")
	var (
		pageNumber uint
		limit      int
		offset     int
	...
	if h.isNoEmptyField(c.Query("page_number")) {
		...
		limit = impl.ReservationsPageLimit
		offset = int((pageNumber - 1) * impl.ReservationsPageLimit)
	}
	...
	reservations, err = h.reservationService.GetByReaderID(c.Request.Context(), readerID, limit, offset)
	if err != nil {
		c.AbortWithStatusJSON(http.StatusInternalServerError, jsondto.ErrorResponse{ErrorMsg: err.Error()})
		return
	}
